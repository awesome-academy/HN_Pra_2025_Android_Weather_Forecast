package com.sun.weatherapp.pressure_detail
...
import com.sun.weatherapp.screen.pressure_detail.PressureDetailContract
import com.sun.weatherapp.screen.pressure_detail.PressureDetailPresenter
...
class PressureDetailPresenterTest {
    private lateinit var presenter: PressureDetailPresenter
    private val mockLocationRepo: LocationRepository = mock()
    private val mockWeatherRepo: WeatherRepository = mock()
    private val mockView: PressureDetailContract.View = mock()

    @Before
    fun setup() {
        presenter = PressureDetailPresenter(mockLocationRepo, mockWeatherRepo)
        presenter.attachView(mockView)
    }

    @Test
    fun `loadPressureDetail should show loading and request location`() {
    ...
    @Test
    fun `refreshData should delegate to loadPressureDetail`() {
    ...
    @Test
    fun `weather success should hide loading and show pressure data`() {
        presenter.loadPressureDetail()
        ...
        verify(mockView).hideLoading()
        verify(mockView).showPressureData(data)
    }
    ...
    fun `weather error should hide loading and show error`() {
        ...
        val error = Exception("pressure error")
        weatherListener.onError(error)

        verify(mockView).hideLoading()
        verify(mockView).showError("pressure error")
    }
}
